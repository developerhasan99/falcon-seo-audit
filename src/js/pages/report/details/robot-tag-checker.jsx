...
const SEORobotsChecker = ({ content }) => {
    ...
    useEffect(() => {
        ...
        const directiveList = content.toLowerCase().split(',').map(d => d.trim());
        ...
        switch (status) {
            ...
            case 'not-available':
                return {
                    color: 'text-red-500',
                    barColor: 'bg-red-500',
                    message: 'No robots meta tag found. Search engines will use default behavior.',
                    icon: X
                };
            default:
                return {
                    ...
                    <p className="text-base relative pl-6">
                        ...
                        <span>
                            {statusInfo.message} {status !== 'not-available' && (
                                <span>
                                    Learn more from <a className="hover:underline newtab" target="_blank" href="https://developers.google.com/search/docs/crawling-indexing/robots-meta-tag" rel="noopener noreferrer">Google</a>.
                                </span>
                            )}
                        </span>
                    </p>
