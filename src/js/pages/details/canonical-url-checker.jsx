...
const CanonicalURLChecker = ({ url, pageUrl }) => {
  ...
  useEffect(() => {
    ...
    if (pageUrl !== url) {
      newIssues.push(
        `Canonical URL does not match the current page URL: ${pageUrl}`
      );
      setStatus("warning");
    ...
    switch (status) {
      ...
      case "warning":
        return {
          ...
          message: "Canonical URL does not match the current page URL.",
          icon: Square,
        };
      case "error":
        return {
      ...
      default:
        return {
          ...
          barColor: "bg-red-500",
          message:
            "Use canonical URLs on your pages to avoid duplicate content issues and improve your website's search engine ranking.",
          icon: X,
        };
    }
  };
