...
const SEOLanguageChecker = ({ lang }) => {
  ...
  useEffect(() => {
    ...
    if (langCode === "en" || langCode === "zh") {
      newIssues.push(
        "Consider using a region-specific language code for better precision"
      );
    }
    ...
    switch (status) {
      ...
      case "not-available":
        return {
          ...
          barColor: "bg-red-500",
          message:
            "No language attribute found. This affects accessibility and SEO.",
          icon: X,
        };
      default:
        return {
  ...
  return (
    <DetailsEntryWrap>
      <div className="flex gap-4 items-center">
        ...
        />
        <div className="text-base font-semibold">Language Attribute</div>
      </div>
          ...
          {status !== "not-available" && status !== "good" && (
            <div className="mt-4 p-4 bg-gray-50 rounded-md">
              ...
              <ul className="space-y-1 text-sm text-gray-600">
                <li>
                  • Use region-specific codes when applicable (e.g., 'en-US'
                  instead of 'en')
                </li>
                <li>• Follow ISO 639-1 language codes for consistency</li>
                <li>
                  • Ensure the language code matches the page's primary content
                </li>
