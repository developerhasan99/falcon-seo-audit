...
function falcon_seo_audit_delete_audit(WP_REST_Request $request)
{
    ...
    if (isset($data['id']) && is_string($data['id']) && !empty($data['id'])) {
        ...
        $report_exists = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $audit_report_table_name WHERE id = %s",
            $audit_id
        ));

        if ($report_exists) {
            // Delete the audit report and related single content report
            $wpdb->delete($audit_report_table_name, ['id' => $audit_id], ['%s']);
            $wpdb->delete($single_content_report_table, ['report_id' => $audit_id], ['%s']);
